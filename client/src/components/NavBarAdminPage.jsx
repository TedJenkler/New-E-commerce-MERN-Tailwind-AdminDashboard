...
function NavBarAdminPage() {
  ...
  // Example breadcrumb based on current location
  const getBreadcrumb = () => {
    const paths = location.pathname.split('/').filter(path => path.trim() !== '');
    return paths.map((path, index) => (
      <React.Fragment key={index}>
        <li className="flex items-center text-blue-gray-900 antialiased font-sans text-sm font-normal leading-normal cursor-pointer transition-colors duration-300 hover:text-light-blue-500">
          <Link to={`/${path}`}>
            <p className="block antialiased font-sans text-sm leading-normal text-blue-900 font-normal opacity-50 transition-all hover:text-blue-500 hover:opacity-100">{path}</p>
          </Link>
    ...
    <nav className="block w-full max-w-full bg-transparent text-white shadow-none rounded-xl transition-all px-0 py-1">
      <div className="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
        <div className="capitalize">
          ...
          <h6 className="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900">{category}</h6>
        </div>
